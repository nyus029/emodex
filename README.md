...
ローカルで MySQL を起動できない場合は、`.env` の `EMODEX_DB_MOCK=true` を設定すると Prisma を使う API をモック応答に切り替えられます。アルバム/グループ周りの API がダミーデータを返し、認証もモックユーザーで通過するため、フロントエンドの UI 挙動を確認できます（本番/ステージングでは `false` のまま運用してください）。
